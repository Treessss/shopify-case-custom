  ...
  initSection(section) {
    ...
    blocks.forEach((block) => {
      block.addEventListener('click', () => {
        ...
        const targetBanner = section.querySelector(`.banner__media-collection[data-index="${blockIndex}"]`);
        if (targetBanner) {
...
let BlsMainMenuShopify = (function () {
  return {
    ...
    initMenu: function () {
      ...
      let horizontalList = document.querySelector(
        ' nav.mobile .horizontal-list'
      );
      let windowWidth = window.innerWidth;
      const header = document.querySelector('header');
      var sectionPbValue = parseInt(
      ...
      HoverIntent(document.querySelectorAll('.menu-parent'), {
        onEnter: function (targetItem) {
          var submenu = targetItem.querySelector('.submenu');
...
var BlsSearchShopify = (function () {
  return {
    ...
    openDefaultSearch: function () {
      ...
      const containsHeaderLayoutOne = document.querySelector('.header__layout-1');
      const containsHeaderLayoutTwo = document.querySelector('.header__layout-2');
      const sf = document.querySelector(containsHeaderLayoutOne ? 'header' : containsHeaderLayoutTwo ? 'header-inner' : 'search-full');
    ...
    closeDefaultSearch: function () {
      console.log('clsoe search default');
      const ps = document.querySelector('.popup-search');
      const sf = document.querySelector('.search-full');
      const overlay_search = document.querySelector('.overlay_search');
      overlay_search.classList.remove('open');
...
class CartNotification extends HTMLElement {
  constructor() {
    ...
    if (this.classList.contains('show_popup')) {
      this.cart_icon = document.querySelector('#cart-icon-bubble');
    ...
    // 新增滑动相关属性
    this.swipeThreshold = 100; // 关闭阈值（MiniCart宽度的50%）
    this.isSwiping = false;
    this.currentTranslateX = 0;

    // 绑定触摸事件
    this.notification.addEventListener('touchstart', this.handleTouchStart.bind(this));
    this.notification.addEventListener('touchmove', this.handleTouchMove.bind(this), { passive: false });
    this.notification.addEventListener('touchend', this.handleTouchEnd.bind(this));
  }
// 修改后的触摸处理方法
handleTouchStart(e) {
  if (!this.notification.classList.contains('open')) return;
  this.isSwiping = true;
  this.startX = e.touches[0].clientX;
  this.startY = e.touches[0].clientY;
  this.minicartWrapper = this.notification.querySelector('.minicart__wrapper');
  this.minicartWidth = this.minicartWrapper.offsetWidth;
  this.swipeDirection = null; // 新增滑动方向状态
  this.minicartWrapper.style.transition = 'none';
}
...
handleTouchMove(e) {
  ...
  if (this.swipeDirection === 'horizontal') {
    e.preventDefault(); // 仅阻止水平滑动默认行为
    this.currentTranslateX = Math.min(deltaX, this.minicartWidth);
    
    // 仅允许右滑
    if (deltaX > 0) {
      this.minicartWrapper.style.transform = `translateX(${this.currentTranslateX}px)`;
    }
  }
  // 垂直方向不处理，允许自然滚动
}
...
handleTouchEnd() {
  ...
  // 只处理水平滑动关闭
  if (this.swipeDirection === 'horizontal') {
    this.minicartWrapper.style.transition = 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)';
    
    if (this.currentTranslateX > this.minicartWidth * 0.3) { // 降低关闭阈值到30%
      this.minicartWrapper.style.transform = `translateX(${this.minicartWidth}px)`;
      setTimeout(() => this.close(), 300);
    } else {
      this.minicartWrapper.style.transform = 'translateX(0)';
    }
  }
  
  this.swipeDirection = null; // 重置方向状态
}
  ...
  close() {
    // 【新增】获取 minicartWrapper 引用（或在 constructor 中统一缓存）
    const minicartWrapper = this.notification.querySelector('.minicart__wrapper');
    
    // 【关键修改】滑动关闭后重置定位样式
    minicartWrapper.style.transition = ''; // 清除内联的 transition 属性
    minicartWrapper.style.transform = '';  // 清除 translateX 偏移
    ...
    setTimeout(() => {
        root.style.removeProperty('padding-right');
        document.documentElement.classList.remove('open-drawer');
        const recommendations = document.querySelector('minicart-recommendations-beside');
        if (recommendations) recommendations.classList.remove('open');
    }, time + 550);

    // 【保持原有逻辑】清理事件和状态
    document.body.removeEventListener('click', this.onBodyClick);
    document.querySelectorAll('.addon').forEach(item => item.classList.remove('is-open'));
}
